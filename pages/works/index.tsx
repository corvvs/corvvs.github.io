...
const TechsList = (props: { techs: string[] }) => {
  return (
    <ul className="flex flex-row flex-wrap gap-1">
      {props.techs.map((tech) => (
        <li key={tech} className="px-2 py-0 border-[1px]">
          {tech}
        </li>
      ))}
    </ul>
...
const WorksListItem = (item: WorkItem) => {
  ...
  return (
    <li className="flex flex-col border-[1px] p-2 w-full sm:max-w-sm gap-2">
    ...
    <div className="flex flex-col gap-4">
      ...
      <ul className="flex flex-row items-start flex-wrap gap-8">
        {props.items.map((item) => (
          <WorksListItem key={item.title} {...item} />
        ))}
      </ul>
    </div>
  );
...
const WorksContent = () => {
  return (
    <>
      <WorksList
        title="個人的なもの"
        items={WorkItemsPrivate}
      />
      <WorksList
        title="CTFコンテスト"
        items={WorkItemsCTFContest}
      />
      <WorksList
        title="42Tokyo Exercises"
        items={WorkItemsFT}
      />
    </>
  );
